...
**Shared agent setup:** this file is the single source of project instructions for
Codex and Claude Code. Codex loads it directly; `.claude/CLAUDE.md` imports it.
Edit shared rules here, never duplicate them in agent-specific files. Paths below
are relative to the repository root; bare reference filenames mean `reference/`.
...
| Recording/replaying live LFS scenarios or reading traces | `simulation_tests/README.md` (the in-game harness; `reference/testing.md` for how it fits the offline suite) |
...
Keep `AGENTS.md` under ~200 lines — it is loaded into every session. Detail belongs
in `reference/`.
